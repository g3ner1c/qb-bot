...
def parse_int_range(int_ranges: list[str]) -> list[int]:
    ...
    -------
        `list[int]`
            A list of integers possible in the given ranges.

    Examples
    --------
    >>> parse_int_range(["2-", "4-5", "8+"])
    [0, 1, 2, 4, 5, 8, 9, 10]
    >>> parse_int_range(["<3", ">5"])
    [0, 1, 2, 6, 7, 8, 9, 10]
    >>> parse_int_range(["<=3", ">=5"])
...
def parse_subcats(subcats: list[str]) -> list[str]:
    ...
    ----------
        subcats : `list[str]`
            A list of categories and aliases. (single words)

            Order does matter, the function will try to match the longest possible subcategory out
            of the given words in `subcats`.
    ...
    -------
        `list[str]`
            Compiled list of all subcategories from the given categories and aliases.
    ...
    >>> parse_subcats(["sci", "us", "hist"])
    ["American History", "Biology", "Chemistry", ...]
    """
...
async def check_answer(
    ...
    """Check if an answer is correct using the qbreader API.
    ...
    -------
        `tuple[str, str | None]`
            A tuple of the directive and prompted responses, if any.
    """
